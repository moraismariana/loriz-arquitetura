...
Promise.all([
  fetch("http://127.0.0.1:8000/sobretexto/1/").then((res) => res.json()),
  fetch("http://127.0.0.1:8000/sobreimagem/1/").then((res) => res.json()),
  fetch("http://127.0.0.1:8000/sobrebg/1/").then((res) => res.json()),
]).then(([textos, imagens, backgrounds]) => {
  // mudar essa linha
  document.getElementById("sobre-content").classList.add("visible");
