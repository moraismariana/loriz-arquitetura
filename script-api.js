...
Promise.all([
  fetch("http://127.0.0.1:8000/paginainicialtexto/1/").then((res) =>
    res.json()
  ),
  fetch("http://127.0.0.1:8000/paginainicialimagem/1/").then((res) =>
    res.json()
  ),
  fetch("http://127.0.0.1:8000/paginainicialbg/1/").then((res) => res.json()),
]).then(([textos, imagens, backgrounds]) => {
  // mudar essa linha
  document.getElementById("pagina-inicial-content").classList.add("visible");
